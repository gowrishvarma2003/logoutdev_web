...
import { ApiError, checkUsernameAvailability, firebaseLogin, registerUser, sendEmailOtp, verifyEmailOtp } from "@/lib/api";
...
export default function SignupPage() {
  ...
  const handleEmailSignup = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const data = await registerUser(name, email, password, username, verification_token);
  ...
  const handleGoogleProfileComplete = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const { available } = await checkUsernameAvailability(username);
      if (!available) throw new Error("This username is already taken.");
      const data = await firebaseLogin(pendingGoogleToken, name, username);
      localStorage.setItem("authToken", data.token);
      localStorage.setItem("currentUser", JSON.stringify(data.user));
      router.push("/feed");
    } catch (error) {
