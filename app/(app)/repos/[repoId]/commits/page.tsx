      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        {author && (
          <div className="flex items-center gap-2">
            <span className="text-sm text-text-muted">Author: <strong className="text-text-primary">{author}</strong></span>
            <Link href={`/repos/${repo.id}/commits?ref=${encodeURIComponent(activeRef)}`} className="rounded-md bg-surface-hover p-1 text-text-muted hover:text-text-primary">
      ...
      {!loading && !error && filteredCommits.length > 0 ? (
        <div className="space-y-6">
          {groupedCommits.map((group) => (
            <div key={group.date}>
              <div className="mb-2 flex items-center gap-2 text-sm text-text-disabled">
                <ClockIcon className="h-4 w-4" />
                <span>Commits on {group.date}</span>
              </div>

              <div className="relative border-l border-border-default ml-2 space-y-4 pb-2">
                {group.commits.map((commit) => (
                  <div key={commit.oid} className="relative pl-6">
                    {/* Timeline dot */}
                    <div className="absolute left-[-5px] top-6 h-2 w-2 rounded-full border border-zinc-500 bg-app" />

                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-lg border border-border-default bg-surface/40 p-3 hover:bg-surface/80 transition-colors">
                      <div className="min-w-0 flex-1">
                        <Link
                          href={`/repos/${repo.id}/commits/${commit.oid}`}
                          className="text-sm font-semibold text-text-primary hover:text-blue-500 hover:underline break-words"
                        >
                          {commit.message}
                        </Link>
                        ...
                        <div className="mt-1 flex items-center gap-2 text-xs text-text-muted">
                          <Link href={authorHref(getAuthorName(commit.author_name))} className="flex items-center gap-1.5 hover:text-blue-400">
                            ...
                            <span className="font-medium text-text-secondary">{getAuthorName(commit.author_name)}</span>
                          </Link>
                          <span>committed</span>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        <div className="flex overflow-hidden rounded-md border border-border-strong bg-surface-hover text-xs text-text-secondary">
                          <button
                            onClick={() => handleCopyHash(commit.oid)}
                            className="border-r border-border-strong px-2 py-1 hover:bg-surface-active"
                            title="Copy full SHA"
                          >
                          ...
                          <Link
                            href={`/repos/${repo.id}/commits/${commit.oid}`}
                            className="px-2.5 py-1 hover:bg-surface-active hover:text-blue-400 font-mono transition-colors"
                          >
                            {commit.short_oid}
                          </Link>
                        </div>
                        <Link
                          ...
                          className="rounded-md border border-border-strong p-1.5 text-text-muted hover:bg-surface-active hover:text-text-primary transition-colors"
          ...
          <div className="mt-8 flex justify-center">
            <div className="inline-flex items-center overflow-hidden rounded-lg border border-border-default bg-surface">
              <button
                onClick={() => setPage(p => Math.max(1, p - 1))}
                disabled={page <= 1}
                className="px-4 py-2 text-sm font-medium text-blue-500 hover:bg-surface-hover disabled:text-text-disabled disabled:hover:bg-transparent transition-colors"
              >
                Newer
              </button>
              <button
                ...
                className="border-l border-border-default px-4 py-2 text-sm font-medium text-blue-500 hover:bg-surface-hover disabled:text-text-disabled disabled:hover:bg-transparent transition-colors"
              >
                Older
              </button>
            </div>
