...
export default function JobPage({ params }: { params: Promise<{ jobId: string }> }) {
  ...
  return <div className="space-y-4"><div><h1 className="text-2xl font-semibold text-text-primary">{job.name}</h1><p className="mt-1 text-sm capitalize text-text-muted">{job.status} · attempt {job.attempt} · {job.image}</p></div><div className="max-h-[65vh] overflow-auto rounded-xl border border-border-default bg-black p-4 font-mono text-xs leading-5">{logs.length ? logs.map((line) => <div key={`${line.sequence}-${line.id}`} className={line.log_type === "stderr" ? "text-rose-300" : line.log_type === "system" ? "text-blue-300" : "text-text-secondary"}><span className="mr-3 select-none text-zinc-700">{String(line.sequence).padStart(5, "0")}</span>{line.content}</div>) : <p className="text-text-disabled">Waiting for logs…</p>}<div ref={end} /></div></div>;
}
