...
export default function RunPage({ params }: { params: Promise<{ repoId: string; runId: string }> }) {
  ...
  return <div className="space-y-6">
    <div className="flex items-start justify-between gap-4"><div><Link href={`/repos/${repoId}/actions`} className="text-sm text-blue-400 hover:underline">← All runs</Link><h1 className="mt-3 text-2xl font-semibold text-white">{run.workflow?.name || "Workflow run"}</h1><p className="mt-1 text-sm text-zinc-400">{run.branch} · <code>{run.commit_sha}</code></p></div><div className="flex gap-2">{active ? <button onClick={() => actionsApi.cancelRun(run.id).then(load)} className="rounded-lg border border-rose-500/30 px-3 py-2 text-sm text-rose-300">Cancel</button> : <button onClick={() => actionsApi.rerunRun(run.id).then(({ run: next }) => { window.location.href = `/repos/${repoId}/actions/runs/${next.id}`; })} className="rounded-lg bg-white px-3 py-2 text-sm font-semibold text-zinc-950">Run again</button>}</div></div>
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-4"><span className="text-xs font-semibold uppercase tracking-wider text-zinc-500">Status</span><p className="mt-2 text-lg font-semibold capitalize text-white">{run.status}</p></div>
    <section className="space-y-3">{run.jobs?.map((job) => <Link key={job.id} href={`/repos/${repoId}/actions/jobs/${job.id}`} className="block rounded-xl border border-zinc-800 p-4 hover:bg-zinc-900/40"><div className="flex justify-between"><span className="font-medium text-zinc-100">{job.name}</span><span className="text-xs font-semibold uppercase text-zinc-400">{job.status}</span></div><div className="mt-3 space-y-1">{job.steps?.map((step) => <div key={step.id} className="flex justify-between text-sm text-zinc-400"><span>{step.name}</span><span>{step.status}{step.exit_code != null ? ` (${step.exit_code})` : ""}</span></div>)}</div>{job.error_message ? <p className="mt-3 rounded-md bg-rose-500/10 px-3 py-2 text-sm text-rose-300">{job.error_message}</p> : null}</Link>)}</section>
    <section className="overflow-hidden rounded-xl border border-zinc-800">
      <div className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900/50 px-4 py-3"><h2 className="text-sm font-semibold text-zinc-200">Complete workflow logs</h2><span className="text-xs text-zinc-500">{logs.length.toLocaleString()} entries</span></div>
      <div className="max-h-[70vh] overflow-auto bg-black p-4 font-mono text-xs leading-5">
        {logs.length ? logs.map((line) => <div key={line.id} className={`grid grid-cols-[minmax(7rem,12rem)_1fr] gap-3 ${line.log_type === "stderr" ? "text-rose-300" : line.log_type === "system" ? "text-blue-300" : "text-zinc-300"}`}><span className="truncate text-zinc-600" title={line.job?.name}>{line.job?.name || "workflow"}</span><span className="whitespace-pre-wrap break-words">{line.content}</span></div>) : <p className="text-zinc-600">{active ? "Waiting for logs…" : "This workflow run did not produce any logs."}</p>}
      </div>
    </section>
  </div>;
}
