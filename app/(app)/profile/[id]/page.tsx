...
function EmptyCTA({ isMe }: { isMe: boolean }) {
  if (!isMe) {
    return (
      <div className="px-5 py-16 text-center">
        <div className="w-12 h-12 rounded-2xl bg-surface border border-border-default flex items-center justify-center mx-auto mb-3">
          <SparklesIcon className="w-5 h-5 text-text-disabled" />
        </div>
  ...
  return (
    <div className="px-5 py-12">
      <div className="rounded-2xl border border-border-default bg-surface/40 p-6 text-center">
        ...
        <h2 className="text-base font-semibold text-text-primary">Build your profile</h2>
        <p className="text-sm text-text-disabled mt-1 max-w-sm mx-auto">
        ...
        <div className="mt-5 grid sm:grid-cols-3 gap-2.5 text-left">
          <Link href="/settings/profile" className="rounded-xl border border-border-default bg-surface/60 p-3 hover:border-border-strong transition-colors group">
            <div className="flex items-center gap-2 text-sm font-medium text-text-primary">
              <CodeBracketIcon className="w-4 h-4 text-violet-400" />
              Add your skills
            </div>
            <p className="text-xs text-text-disabled mt-1">Pin your stack so others can find you.</p>
          </Link>
          <Link href="/settings/profile" className="rounded-xl border border-border-default bg-surface/60 p-3 hover:border-border-strong transition-colors group">
            <div className="flex items-center gap-2 text-sm font-medium text-text-primary">
            ...
            <p className="text-xs text-text-disabled mt-1">Showcase up to 3 spaces you build in.</p>
          </Link>
          <Link href="/launches/new" className="rounded-xl border border-border-default bg-surface/60 p-3 hover:border-border-strong transition-colors group">
            <div className="flex items-center gap-2 text-sm font-medium text-text-primary">
              <TrendingUpIcon className="w-4 h-4 text-emerald-400" />
              Ship a launch
            </div>
            <p className="text-xs text-text-disabled mt-1">Grow your proof-of-work score.</p>
          </Link>
        </div>
        <Link
          href="/settings/profile"
          className="mt-5 inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary-hover transition-colors"
        >
          Edit profile
        </Link>
      </div>
  ...
  return (
    <div className="px-5 py-6 space-y-7">
      ...
      {signals ? (
        <section id="proof-of-work" className="space-y-3">
          ...
          {career_summary && career_summary.strongest_stacks.length > 0 ? (
            <div className="rounded-2xl border border-border-default bg-surface/40 p-4">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-text-disabled mb-2.5">
                Strongest stacks
              </p>
      ...
      {career_summary && career_summary.timeline.length > 0 ? (
        <section className="rounded-2xl border border-border-default bg-surface/60 p-4">
          <div className="flex flex-wrap items-center gap-2">
            <h2 className="text-base font-semibold text-text-primary">Career graph</h2>
          ...
          <div className="mt-5 relative">
            {/* vertical rail */}
            <div className="absolute left-[15px] top-2 bottom-2 w-px bg-surface-hover" />
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 pt-0.5">
                      ...
                      <p className="mt-1 text-[11px] text-text-disabled flex items-center gap-1">
