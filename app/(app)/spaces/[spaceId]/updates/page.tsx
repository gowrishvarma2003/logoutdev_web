...
export default function UpdatesPage({
  ...
  return (
    <div>
        ...
        action={
          isMember && (
            <button
              ...
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary-hover transition-colors"
            >
      ...
      {activeWorkItemId ? (
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border-default bg-surface/20 px-4 py-3">
          <div>
      ...
      {/* ── Composer ──────────────────────────────────────────────────────── */}
      {showComposer && (
        <form onSubmit={handlePost} className="px-4 py-4 border-b border-border-default space-y-3 bg-surface/30">
          <div className="flex gap-2 flex-wrap">
            <select
              value={type}
              onChange={(e) => setType(e.target.value as UpdateType)}
              className="px-2.5 py-1.5 rounded-lg bg-surface border border-border-default text-xs text-text-primary focus:outline-none focus:border-border-strong"
            >
              {UPDATE_TYPES.map((t) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            <input
              ...
              className="px-3 py-2 rounded-lg bg-surface border border-border-default text-xs text-text-primary placeholder:text-text-disabled focus:outline-none focus:border-border-strong transition-colors"
            />
            <input
              type="text"
              value={nextUp}
          ...
          <textarea
            ...
            rows={2}
            className="w-full px-3 py-2 rounded-lg bg-surface border border-border-default text-xs text-text-primary placeholder:text-text-disabled focus:outline-none focus:border-border-strong resize-none transition-colors"
          />
      ...
      {(page > 1 || updates.length >= 20) && (
        <div className="flex items-center justify-center gap-3 py-4">
          <button
            ...
            disabled={page <= 1}
            className="px-3 py-1.5 rounded-lg text-xs font-medium bg-surface text-text-muted hover:bg-surface-hover disabled:opacity-40 transition-colors"
          >
            Previous
          </button>
