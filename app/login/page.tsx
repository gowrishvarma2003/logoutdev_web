...
import { firebaseLogin, loginUser } from "@/lib/api";
import { loginWithEmail, loginWithFirebaseCustomToken, loginWithGoogle } from "@/lib/firebase";
    ...
    try {
      ...
      try {
        ...
        if (!(firebaseError instanceof FirebaseError)
          || !["auth/invalid-credential", "auth/user-not-found", "auth/wrong-password"].includes(firebaseError.code)) {
          throw firebaseError;
        }

        // A valid legacy password performs a one-time, in-place Firebase
        // migration and returns a custom token for the new identity.
        const legacySession = await loginUser(email, password);
        if (!legacySession.firebase_custom_token) throw firebaseError;
        idToken = await loginWithFirebaseCustomToken(legacySession.firebase_custom_token);
      }
