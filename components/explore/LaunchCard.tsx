...
export default function LaunchCard({ item }: LaunchCardProps) {
  ...
  }[firstLetter] || "bg-surface-hover/50 text-text-secondary border-border-strong/30";
  ...
  }[stage.toLowerCase()] || "bg-surface-hover/40 text-text-muted border-border-strong/30";

  return (
    <div className="group flex h-full items-stretch justify-between gap-4 rounded-2xl border border-border-default/60 bg-surface/30 p-5 backdrop-blur-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-border-strong/60 hover:bg-surface/60">
      <div className="flex min-w-0 flex-1 gap-4">
        ...
        <div className="min-w-0 flex-1 flex flex-col justify-between">
          ...
          {/* Stacks & Creator */}
          <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-[10px] text-text-disabled">
            {item.meta.byline ? (
              <span className="font-medium text-text-muted">{item.meta.byline}</span>
            ) : null}
            {item.tags && item.tags.length > 0 ? (
              <div className="flex flex-wrap gap-1">
                {item.tags.slice(0, 3).map((tag) => (
                  <span key={tag} className="text-text-disabled bg-surface-hover/20 px-1 py-0.5 rounded">
                    #{tag}
                  </span>
                ))}
              </div>
      ...
      <div className="flex flex-col items-center justify-between border-l border-border-default/40 pl-4 min-w-[65px]">
        {/* Vote container */}
        <Link
          href={item.href}
          className="flex w-full flex-col items-center justify-center rounded-xl border border-border-default bg-app/40 py-2 transition-colors hover:border-border-strong hover:bg-surface/80 group/vote"
        >
          <ArrowUpIcon className="h-4 w-4 text-text-muted group-hover/vote:text-sky-400 transition-colors" />
          <span className="mt-1 text-xs font-bold text-text-secondary group-hover/vote:text-text-primary">
            {upvotes}
          </span>
        </Link>
        <span className="text-[9px] font-medium text-text-disabled uppercase tracking-wider mt-2">
