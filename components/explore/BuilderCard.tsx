...
export default function BuilderCard({ item }: BuilderCardProps) {
  ...
  const badgeStyles = {
    ...
    Early: "bg-zinc-800/40 text-zinc-400 border-zinc-700/30",
  }[proofBand as "Strong" | "Growing" | "Early"] || "bg-zinc-800/40 text-zinc-400 border-zinc-700/30";

  return (
    <div className="group flex h-full flex-col rounded-2xl border border-zinc-800/60 bg-zinc-900/30 p-5 backdrop-blur-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-zinc-700/60 hover:bg-zinc-900/60">
      {/* Header Profile Row */}
      <div className="flex items-start justify-between gap-3">
        <Link href={item.href} className="flex min-w-0 items-center gap-3">
          <Avatar user={user} size="md" className="ring-2 ring-zinc-800/40 transition-transform group-hover:scale-105" />
          <div className="min-w-0">
            <h3 className="line-clamp-1 text-sm font-semibold text-zinc-200 group-hover:text-white transition-colors">
              {item.title}
            </h3>
            {item.meta.byline ? (
      ...
      {/* Headline / Bio */}
      <Link href={item.href} className="mt-3.5 block flex-1 text-xs leading-relaxed text-zinc-400 transition-colors hover:text-zinc-300 line-clamp-2">
      ...
      {item.tags && item.tags.length > 0 ? (
        <div className="mt-4 flex flex-wrap gap-1.5">
          {item.tags.slice(0, 4).map((tag) => (
            <span
              key={`${item.id}:tag:${tag}`}
              className="rounded-lg border border-zinc-800/80 bg-zinc-800/30 px-2 py-0.5 text-[10px] font-medium text-zinc-300 transition-colors hover:bg-zinc-800/60"
            >
              {tag}
            </span>
          ))}
      ...
      {item.meta.collaboration_label || item.rank_explanation?.reasons?.[0] ? (
        <div className="mt-4 flex flex-col gap-1 border-t border-zinc-800/40 pt-3">
      ...
      {/* Stats Footer */}
      <div className="mt-3 flex items-center justify-between border-t border-zinc-800/40 pt-3 text-[10px] text-zinc-500">
