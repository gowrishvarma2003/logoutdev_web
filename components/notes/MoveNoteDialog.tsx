    ...
    <Modal open={open} onClose={onClose} title="Move to folder">
      <div className="max-h-72 space-y-0.5 overflow-y-auto">
        <button
          ...
          className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-left text-sm text-zinc-300 transition-colors hover:bg-zinc-800 hover:text-white"
        >
        ...
        {folders.folders.length === 0 ? (
          <p className="px-3 py-2 text-sm text-zinc-500">No folders yet — create one from the sidebar.</p>
        ) : null}
      </div>
    </Modal>
  );
