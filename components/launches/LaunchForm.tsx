...
export default function LaunchForm({
  ...
  const [developmentStage, setDevelopmentStage] = useState<LaunchDevelopmentStage>(initialLaunch?.development_stage ?? "mvp");
  const [demoUrl, setDemoUrl] = useState(initialLaunch?.demo_url ?? "");
  const [websiteUrl, setWebsiteUrl] = useState(initialLaunch?.website_url ?? "");
  const [githubUrl, setGithubUrl] = useState(initialLaunch?.github_url ?? "");
  const [docsUrl, setDocsUrl] = useState(initialLaunch?.docs_url ?? "");
  const [collaborationMode, setCollaborationMode] = useState<"off" | "looking">(initialLaunch?.collaboration_mode ?? "off");
  const [collaborationNote, setCollaborationNote] = useState(initialLaunch?.collaboration_note ?? "");
  const [collaborationRoles, setCollaborationRoles] = useState(joinRoles(initialLaunch));
  const [linkedSpaceId, setLinkedSpaceId] = useState(initialLaunch?.linked_space?.id ?? initialLaunch?.linked_space_id ?? "");
  const [screenshots, setScreenshots] = useState(joinScreenshots(initialLaunch));
  const [techStack, setTechStack] = useState(joinTechStack(initialLaunch));
  const [localError, setLocalError] = useState<string | null>(null);
  const linkedSpaceOptions = mySpacesData?.spaces ?? [];
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const screenshotList = screenshots.split("\n").map((item) => item.trim()).filter(Boolean);
    const techStackList = techStack.split(",").map((item) => item.trim()).filter(Boolean);
    const roles = collaborationRoles.split(",").map((item) => item.trim()).filter(Boolean);

    if (screenshotList.length > 6) {
      setLocalError("Use at most 6 screenshot URLs.");
      return;
    }

    if (techStackList.length > 12) {
      setLocalError("Use at most 12 tech stack items.");
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="sm:col-span-2">
          <label className="mb-1.5 block text-sm font-medium text-zinc-300">Product name</label>
          <input value={name} onChange={(e) => setName(e.target.value)} className="w-full rounded-xl border border-zinc-800 bg-zinc-900 px-3 py-2.5 text-sm text-white focus:border-zinc-600 focus:outline-none" />
        </div>
        <div className="sm:col-span-2">
          <label className="mb-1.5 block text-sm font-medium text-zinc-300">Tagline</label>
          <input value={tagline} onChange={(e) => setTagline(e.target.value)} className="w-full rounded-xl border border-zinc-800 bg-zinc-900 px-3 py-2.5 text-sm text-white focus:border-zinc-600 focus:outline-none" />
        </div>
        <div>
          <label className="mb-1.5 block text-sm font-medium text-zinc-300">Product type</label>
          <select value={productType} onChange={(e) => setProductType(e.target.value as LaunchProductType)} className="w-full rounded-xl border border-zinc-800 bg-zinc-900 px-3 py-2.5 text-sm text-white focus:border-zinc-600 focus:outline-none">
            <option value="web-app">Web App</option>
            <option value="mobile-app">Mobile App</option>
            <option value="developer-tool">Developer Tool</option>
            <option value="api">API</option>
