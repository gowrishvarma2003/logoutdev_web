  ...
  return (
    <div className="flex gap-3 group">
      ...
      <div className="flex flex-col items-center">
        ...
        <div className="w-px flex-1 bg-surface-hover mt-2 min-h-[16px]" />
      </div>

      <div className="pb-5 flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-[11px] font-medium text-text-disabled uppercase tracking-wide">
            {config.label}
          </span>
        ...
        <Link
          ...
          aria-label={title}
        >
          <p className="text-sm text-text-secondary group-hover/link:text-text-primary transition-colors line-clamp-2 leading-relaxed">
            {title}
          </p>
          {subLabel && (
            <p className="text-xs text-text-disabled mt-0.5">in {subLabel}</p>
          )}
          {item.stats ? <p className="mt-1 text-xs text-text-disabled">{item.stats}</p> : null}
        </Link>
        {item.linked_entity ? (
          <div className="mt-3">
